...
# OUTPUT_CSV = os.path.join(BASE_DIR, "skin_tone_features1.csv")
# RESIZE_DIM = (512, 512)
# try:
...
# # Save features-only CSV containing `isic_id` and computed features
...
df1 = pd.read_csv("skin_tone_features.csv")
df2 = pd.read_csv("skin_tone_features1.csv")
df2["fitzpatrick_skin_type"] = df2["isic_id"].map(
    df1.set_index("isic_id")["fitzpatrick_skin_type"]
)
# df2.join(df1.set_index("fitzpatrick_skin_type"), on="isic_id", how="left", rsuffix="_new").to_csv("skin_tone_features_final.csv", index=False)
df2.to_csv("skin_tone_features_final.csv", index=False)